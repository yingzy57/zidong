...
df['昵称']=df['昵称'].replace('	', '', regex=True)
data_res=pd.merge(data[['昵称','客户周数']],df,left_on='昵称',right_on='昵称')
...
df_youxiao=pd.merge(data[['昵称','客户周数']],df_youxiao,left_on='昵称',right_on='昵称')
...
with pd.ExcelWriter(f'{file_root}/按周结果（句子验证）.xlsx') as writer:
